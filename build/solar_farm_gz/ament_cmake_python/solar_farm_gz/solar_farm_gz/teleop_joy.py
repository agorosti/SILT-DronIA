...
USB gamepad flies the simulated aircraft exactly as a transmitter would fly the
physical one.

Why not MAVROS
--------------
MAVROS is the usual answer and it works, but it is a large dependency to add
for what is ultimately four numbers and a heartbeat. Talking to SITL directly
with pymavlink keeps the runtime requirements to `joy` (already part of a
standard ROS 2 desktop install) plus pymavlink, and removes a whole package
from the list of things that can be version-mismatched on the client's machine.

Channel mapping follows ArduPilot's RC defaults:

    CH1 roll     CH2 pitch     CH3 throttle     CH4 yaw

Axis assignments default to the Mode 2 layout that an Xbox/PlayStation-style
pad presents through the `joy` driver: left stick is throttle/yaw, right stick
is pitch/roll. Every index is a ROS parameter, because pads differ and the
client should not have to edit source to fly.

    ros2 run solar_farm_gz teleop_joy
    ros2 run solar_farm_gz teleop_joy --ros-args -p master:=tcp:127.0.0.1:5760

Flight modes are on buttons rather than a switch: a pad has no three-position
mode switch, and LOITER/ALT_HOLD/STABILIZE covers everything an inspection
flight needs.
"""
...
# ArduPilot reads RC as PWM microseconds. 1500 is centre stick; throttle is the
# exception, where centre means "hold current altitude" in ALT_HOLD and LOITER
# but means half power in STABILIZE.
...
class TeleopJoy(Node):
    ...
    def __init__(self):
        ...
        # Sticks are noisy at rest; without a deadzone the aircraft creeps.
        ...
        # source_system MUST match ArduPilot's SYSID_MYGCS (default 255).
        # Overrides from any other system id are accepted onto the link and
        # then silently discarded: RC_CHANNELS keeps reporting the unmodified
        # inputs and the aircraft simply does not respond, with no error
        ...
        # ArduPilot reverts to failsafe if overrides stop arriving, so the last
        # command is republished on a timer rather than only on joystick
        # events. A pad that is held still produces no messages at all.
    ...
    def _shape(self, v):
        """Deadzone then cubic expo, so small stick movements are gentle."""
    ...
    def publish_rc(self):
        r, p, t, y = self._last
        # 0 leaves a channel untouched, so channels 5-8 stay under whatever
        # the flight controller or a GCS has set them to.
