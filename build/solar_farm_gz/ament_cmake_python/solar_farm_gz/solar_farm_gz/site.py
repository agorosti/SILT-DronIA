"""Site infrastructure: perimeter fencing, access roads and inverter stations.

Phase 1 deliberately shipped panels and terrain only. This module adds the rest
of what a real PV site carries, for the establishing shots and the "wow" pass,
without touching any Phase 1 asset.

Two constraints shaped everything here:

* **Seed stability.** Infrastructure draws from its own RNG stream, so turning
  it on or off cannot shift a single table. A given seed produces the same farm
  either way, which keeps Phase 1 worlds reproducible and lets the two be
  rendered as a matched pair.

* **Triangle budget.** The 1000-module world already holds real time by merging
  geometry rather than instancing it, and infrastructure has to respect the same
  rule. A 420 m perimeter at 3 m post spacing is ~140 posts; as individual
  models that is 140 more entities for the physics broadphase and the render
  queue to walk every frame. Merged into one mesh it is a single draw call, so
  that is what happens below. The chain-link fabric is one alpha-textured quad
  per side rather than modelled wire, for the same reason.
"""
...
POST_SIDE = 0.06         # m, square section
ROAD_WIDTH = 4.00        # m, wide enough for a service vehicle
ROAD_Z = 0.02            # m, lifted off the ground plane to avoid z-fighting

# Inverter/transformer skid. Real central inverters for a farm this size are
# roughly container-scale; these are sized to a small skid-mounted unit.
...
def _chainlink_rgba(rng, px=512, pitch=42, wire=4):
    """Tileable chain-link fabric as RGBA, alpha cut to the wire.

    The diamond lattice is two families of diagonal lines, so it falls out of
    the two diagonal coordinates (x+y) and (x-y) taken modulo the mesh pitch.
    Working in the diagonal basis rather than drawing wires individually is
    what keeps this exactly tileable.
    """
    ...
    # Galvanised steel, with enough tonal noise that a long fence run does not
    # read as a flat grey screen at distance.
...
def _housing(rng, px=512):
    """Painted sheet-steel enclosure with faint vertical panel seams."""
...
def write_fence_obj(path, x0, y0, x1, y1):
    ...
    UVs tile along the run at one mesh repeat per 1.4 m, so the diamond size
    stays physically consistent regardless of how long the side is.

    Each side carries its own outward normal. This is not a detail that can be
    skipped: the fabric is vertical, so a shared up-facing normal leaves every
    panel lit as though it were floor, and the fence renders black or vanishes
    entirely. `double_sided` in the material controls culling, not shading, and
    does not compensate.
    """
...
def write_posts_obj(path, x0, y0, x1, y1):
    """Line posts merged into one mesh."""
    from . import pv_mesh
    verts, faces = [], []
    h = FENCE_HEIGHT + 0.08
    s = POST_SIDE
    ...
    # Top rail. Chain-link fabric is mostly holes, so at survey altitude its
    # alpha texture mips away to nothing and the perimeter reads as a line of
    # disconnected posts. A real security fence carries a rail along the top,
    # and that continuous horizontal line is what actually makes a fence
    # legible from the air — so it earns its four boxes.
    r = 0.05
    rz = FENCE_HEIGHT
...
def road_rect(x0, y0, x1, y1):
    """Centreline of the ring road, inset from the fence.

    The road is drawn about this centreline, so the inset has to clear half a
    road width plus a margin — otherwise the carriageway straddles the fence
    line and gravel renders on both sides of it.
    """
...
def write_road_obj(path, x0, y0, x1, y1):
    """Perimeter ring road, set just inside the fence line.

    Four rectangles rather than a true mitred ring: the corners overlap, which
    is invisible on a flat gravel texture and avoids the mitre maths.
    """
    ...
    def strip(ax, ay, bx, by, horizontal):
        ...
        Both branches must wind counter-clockwise seen from above, or the quad
        faces the ground and is backface-culled — it disappears while its
        neighbours on the other two sides render normally, which looks like a
        placement bug rather than a winding one.
        """
        if horizontal:
...
def inverter_positions(x0, y0, x1, y1, n):
    """Evenly spaced down the inside edge of the west road.

    Takes the fence rectangle and derives the road from it, so callers only
    ever deal in one coordinate system.
    """
...
def sdf(asset_uri, x0, y0, x1, y1, n_inverters, shadows):
    ...
    <!--
      Fabric is alpha-cut rather than modelled wire. double_sided is required:
      a single-sided quad is invisible from outside the perimeter, which reads
      as a fence that disappears when the drone flies past it.
    -->
