...
aircraft in GUIDED mode at the survey parameters the client actually flew
(8 m altitude, 1.5 m/s cruise), and both cameras record what happens. If the
controller wobbles, the video shows it.
...
* **The drone is inlined, not included.** The chase camera has to move with the
  aircraft, and repositioning a separate static model through the set_pose
  service costs a subprocess round-trip per frame — far too slow at 20+ fps.
  Inlining the model lets the chase camera hang off base_link on a fixed joint,
  so it tracks for free. The shipped model.sdf is left untouched; the extra
  camera exists only in the temporary capture world.

* **The aircraft is spawned nose-along the rows** and flown with body-frame
  velocity. ArduPilot's NED axes and Gazebo's world axes differ by a rotation
  that is easy to get wrong; commanding MAV_FRAME_BODY_NED sidesteps the
  conversion entirely, and a nose-forward transect is also what a chase camera
  wants to look at.

* **Frames stream to the encoder.** A 40 s flight is ~900 frames; buffering
  1920x1080 RGB would be about 5.6 GB, so each composited frame is written and
  dropped.
"""
...
# Chase camera: behind and above, looking forward and slightly down. Offsets
# are in the aircraft body frame, so this holds through turns.
CHASE_BACK = 7.0
CHASE_UP = 2.6
...
def build_capture_world(world_path, model_path, spawn, chase_wh, rate, outdir):
    """Farm world + the drone inlined with a chase camera bolted on."""
    world = open(world_path).read()
    model = open(model_path).read()

    # Strip the model's own sdf envelope so the <model> can be embedded.
...
def _default_model():
    """Locate x500_rgb/model.sdf in an installed package, else in the source
    tree. The two layouts differ: installed Python lands in
    lib/pythonX/site-packages, which is nowhere near share/."""
    try:
...
def fly(mav, alt, speed, duration, on_tick):
    """Take off, then hold a body-forward cruise, calling on_tick throughout."""
    from pymavlink import mavutil

    # Wait for a position solution before touching the mode or arming. A cold
    # SITL needs the best part of a minute for GPS and the EKF to settle, and
    # arming before that fails the prearm checks rather than waiting politely.
    ...
    # Prearm can still refuse for a few seconds after the fix appears, so the
    # arm request is retried rather than sent once.
    ...
    while time.time() - t0 < 90 and not mav.motors_armed():
        ...
        for _ in range(10):
            ...
            # Surface the refusal reason. "PreArm: ..." is the only thing that
            # tells you why, and swallowing it turns a one-line parameter fix
            # into an afternoon.
    ...
    # Climb out before recording: the first seconds are a vertical ascent with
    # nothing but grass underneath, which makes a poor opening shot.
    t0 = time.time()
    ...
    # Velocity-only mask, body frame. Resent continuously: ArduPilot times a
    # guided setpoint out after a few seconds and would stop and hover.
    ...
    while time.time() - t0 < duration:
        ...
        # Non-blocking: the recording loop must not stall waiting on telemetry,
        # or the video drops frames whenever a packet is late.
        m = mav.recv_match(type='GLOBAL_POSITION_INT', blocking=False)
        if m:
            hud["alt"] = m.relative_alt / 1000.0
            hud["spd"] = math.hypot(m.vx, m.vy) / 100.0
            # Derived from the real camera geometry rather than hard-coded, so
            # the overlay stays honest if the aircraft drifts off altitude.
...
def _shadowed(img, text, org, scale, colour, thick=1):
    """Text with a dark offset copy underneath.

    The footage is a mix of bright grass and near-black panels, so plain white
    text is unreadable over roughly half of every frame."""
    import cv2
...
def composite(chase, nadir, inset_w, hud=None, title=None):
    """Chase view full frame, nadir feed inset bottom-right, telemetry HUD."""
    import cv2
    out = chase.copy()
    h, w = out.shape[:2]
    ...
    if hud:
        # Fixed-format so the numbers do not jitter horizontally frame to
        # frame, which reads as sloppy even when the values are correct.
        ...
        # Skipped while the title band is up, which would otherwise sit on top
        # of it.
        if not title:
...
def main(argv=None):
    ...
    # Nose along +y so the transect runs the length of the rows.
    ...
    # Model URIs inside the farm world resolve against the world directory;
    # the drone is inlined so it needs no resource entry of its own.
    try:
        from . import gpu
    ...
    # ASCII only: OpenCV draws with Hershey vector fonts, which have no glyphs
    # beyond ASCII and render an em-dash or middle dot as "???".
    ...
    def on_tick(elapsed, hud):
        # Drive off the chase camera's frame counter so each recorded frame is
        # a genuinely new render rather than the same one sampled twice.
